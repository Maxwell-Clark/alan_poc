...
import { useMessageStore } from '@/stores/alanStore';
...
var msgStore = useMessageStore();
const alanBtn = instance.appContext.config.globalProperties.$alanBtnInstance;


watch(() => msgStore.text, (newValue, oldValue) => {
...
const sendText = async () => {
  await alanBtn.sendText(question.value);
}
const getAnswer = async (message) => {
  try {
    if(question.value == "") {
      question.value = msgStore.question
    }
    loading.value = true;
    wrapper.value.push({
      isAi: false,
    ...
    console.log('got here bitches', question.value)

    setTimeout(function(){
      wrapper.value.pop();
      wrapper.value.push({
        isAi: true,
        value: msgStore.text.trim(),
      });
      console.log("get answer " + msgStore.text)
...
onMounted(() => {
  getQuestions();
  var msgStore = useMessageStore();
  console.log("TEXT"+ msgStore.text)
});
</script>
